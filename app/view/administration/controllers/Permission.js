/**
 * Grid to change the permission for a controller for multiple roles
 *
 * Change for a complete controller means, that the rules will apply on
...
/**
 * @class Webdesktop.view.administration.controllers.Permission
 * @extends Ext.grid.Panel
 * @alias administration_controllerpermission
 ...
 */
Ext.define('Webdesktop.view.administration.controllers.Permission', {
    extend : 'Ext.grid.Panel',
    alias  : 'widget.administration_controllerpermission',
    initComponent:function() {
        ...
        store = Ext.create('Webdesktop.store.administration.Permissions', {
            ...
            storeId: 'Webdesktop.store.administration.Permission-controller-' + me.record.get('id')
        });
        // set the proxy API: custom implementation, see comments in the permission store
        store.setProxyApi({
            read: store.getProxy().api.base + 'loadControllerPermissions',
            update: store.getProxy().api.base + 'saveControllerPermissions'
        });
        store.load({
            params: {
                cId: me.record.get('id')
            }
        });
